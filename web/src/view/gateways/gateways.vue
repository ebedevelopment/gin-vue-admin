  ...
  <div>
    ...
    <div class="gva-table-box">
      <div class="gva-btn-list">
        ...
        >
        <el-popover v-model:visible="deleteVisible" placement="top" resizable>
      ...
      >
        ...
        <el-table-column
          align="left"
          label="domainNameService"
          prop="domainNameService"
          resizable
        />
        ...
        <el-table-column align="left" label="cols" prop="cols" resizable />
        <el-table-column align="left" :label="t('general.operations')">
          <template #default="scope">
            <el-button
              ...
              size="small"
              class="table-button"
              @click="updateGatewaysFunc(scope.row)"
    ...
    >
      <el-form :model="formData" label-position="left" label-width="160px">
        <el-form-item label="domainNameService:">
          <el-input
            v-model="formData.domainNameService"
            clearable
...
const onDelete = async () => {
  ...
  const res = await deleteGatewaysByIds({ ids });
  if (res.code === 0) {
    ElMessage({
      type: "success",
...
// 更新行
const updateGatewaysFunc = async (row) => {
  const res = await findGateways({ ID: row.ID });
  type.value = "update";
  if (res.code === 0) {
...
// 删除行
const deleteGatewaysFunc = async (row) => {
  const res = await deleteGateways({ ID: row.ID });
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: t("general.deleteSuccess"),
