<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="password field" prop="password" width="120" />
        <el-table-column align="left" label="status field" prop="status" width="120" />
        <el-table-column align="left" label="lastLogin field" prop="lastLogin" width="120" />
        <el-table-column align="left" :label="t('general.operations')">
            <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateUserLoginFunc(scope.row)">{{ t('general.change') }}</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" :title="t('general.popUpOperation')">
      <el-form :model="formData" label-position="right" label-width="80px">
        ...
        <el-form-item label="lastLogin field:">
          <el-date-picker v-model="formData.lastLogin" type="date" style="width:100%" :placeholder="t('general.selectDate')" clearable />
        </el-form-item>
      </el-form>
      <template #footer>
...
const getTableData = async() => {
  const table = await getUserLoginList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteUserLoginByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
...
// 更新行
const updateUserLoginFunc = async(row) => {
    const res = await findUserLogin({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserLoginFunc = async (row) => {
    const res = await deleteUserLogin({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: t('general.deleteSuccess')
